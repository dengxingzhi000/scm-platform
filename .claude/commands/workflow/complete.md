...
提交前必须通过以下自动化检查。**这些检查在 `git commit` 时由 pre-commit 钩子自动执行**,无需手动运行。

### 敏感信息检测

自动扫描暂存区文件,阻止包含以下内容的提交:

- 硬编码密码/密钥: `password=`, `secret=`, `apiKey=`, `privateKey=`
- AWS/GCP/Azure 凭证: `AKIA`, `AIza`, `-----BEGIN RSA PRIVATE KEY-----`
- 数据库连接串中的明文密码: `jdbc:mysql://user:password@`
- JWT 密钥: `jwt.secret=`, `JWT_SECRET=`
- 内部 IP 地址: `192.168.x.x`, `10.x.x.x`, `172.16-31.x.x`

```bash
# .git/hooks/pre-commit 或通过 Husky 执行
# 检测敏感信息
SECRETS_PATTERN='(password|secret|apiKey|privateKey|AKIA|AIza|BEGIN RSA PRIVATE KEY)\s*[=:]'
if git diff --cached --diff-filter=ACM | grep -iE "$SECRETS_PATTERN"; then
    echo "❌ 检测到可能的敏感信息,请使用环境变量或配置中心管理"
    exit 1
fi
```
...
for file in $(git diff --cached --name-only --diff-filter=ACM); do
    ...
    if [ "$file_size" -gt "$MAX_FILE_SIZE" ]; then
        echo "❌ 文件 $file 超过 5MB,请使用 Git LFS 管理大文件"
        exit 1
    fi
done
```
...
- 编译产物: `target/`, `build/`, `*.class`, `*.jar`
- 日志文件: `*.log`
- 系统文件: `.DS_Store`, `Thumbs.db`
- 环境配置: `.env` (应使用 `.env.example`)
...
**多模块项目:** 在 Body 中注明影响的模块,例如:
```
影响模块: scm-order, scm-inventory
```
...
- **Body:** 使用 PR 模板,包含
    - 变更说明:做了什么改动
    - 测试说明:如何验证这些改动
    - 关联 Issue: `Closes #123` (多个 Issue 使用 `Refs #123, #456`)
    - 影响模块:列出本次变更涉及的模块
    - 截图/演示(如涉及UI变更)
...
- [ ] 是否关联了 Issue (`Closes #` 或 `Refs #`)
- [ ] CI 测试是否全部通过
- [ ] 代码覆盖率是否 ≥ 80% (关键模块 ≥ 90%)
- [ ] Checkstyle/SpotBugs 是否无严重问题
- [ ] 依赖安全扫描是否通过
- [ ] 是否指定了 Reviewer
- [ ] 分支是否基于最新的主干 (无冲突)
...
- SonarQube 代码质量门禁
- OWASP 依赖漏洞扫描
- 敏感文件/路径 PR 阻断 (如 `*.key`, `*.pem`, `docker-compose*.yml` 中的密码)
...
- [ ] 至少2名审查者批准
- [ ] 代码冲突已解决
- [ ] 分支基于最新的主干分支 (合并前必须 rebase 到最新 main)

**合并策略:**
- **主干分支 (main/master):** 强制使用 `Squash and Merge`,保持历史清晰
- **长期维护分支:** 使用 `Rebase and Merge`,保留详细提交历史用于追溯
- **热修复分支:** 使用 `Squash and Merge`,快速合入主干

**冲突处理:**
- 合并前 CI 自动检测冲突,如有冲突则阻断合并
- 提交者必须先 `git rebase main` 解决冲突后重新推送
...
**多模块项目:** 缩短模块名,使用缩写或关键字段:
- `feat/usr-jwt-#123` (user-service → usr)
- `fix/ord-npe-#456` (order → ord)
- `feat/inv-reserve-#789` (inventory → inv)
...
- 使用 `Refs #issue` 引用但不关闭 Issue (多 Issue 依赖时使用)
...
影响模块: scm-order, scm-common/data
性能提升:
...
```
feat(inventory): 实现库存预留和扣减功能

- 实现Redis Lua脚本原子扣减
- 添加库存预留超时自动释放
- 实现Seata TCC分布式事务

影响模块: scm-inventory, scm-order
测试覆盖率: 88%
...
| 模块类型 | 最低覆盖率 | 说明 |
|---------|-----------|------|
| 关键业务模块 (order, finance, payment) | ≥ 90% | 必须有集成测试 + contract 测试 |
| 普通业务模块 (product, warehouse, logistics) | ≥ 80% | 单元测试 + 关键路径集成测试 |
| 基础设施模块 (common, gateway) | ≥ 70% | 单元测试为主 |
| 配置/启动类 | 不要求 | Application 类、配置类 |

### 关键模块测试要求

**订单模块 (scm-order):**
- 单元测试: 状态机流转、金额计算、幂等性
- 集成测试: 创建订单 → 扣减库存 → 支付 → 发货 全链路
- Contract 测试: 与 inventory、payment 的 RPC 接口契约

**库存模块 (scm-inventory):**
- 单元测试: Redis Lua 脚本原子性、并发扣减
- 集成测试: 预留 → 扣减 → 释放 全流程
- 压力测试: 高并发场景下的库存一致性
...
if git diff --cached --diff-filter=ACM | grep -iE "$SECRETS_PATTERN" > /dev/null 2>&1; then
    echo "❌ 检测到可能的敏感信息,请使用环境变量或配置中心管理"
    echo "   如果是误报,使用 git commit --no-verify 跳过"
    exit 1
fi
...
for file in $(git diff --cached --name-only --diff-filter=ACM); do
    if [ -f "$file" ]; then
        ...
        if [ "$file_size" -gt "$MAX_FILE_SIZE" ]; then
            echo "❌ 文件 $file 超过 5MB ($(( file_size / 1024 / 1024 ))MB)"
...
BLOCKED_PATTERNS=(".idea/" ".vscode/" "*.iml" "target/" "build/" "*.class" "*.jar" "*.log" ".DS_Store" "Thumbs.db" ".env")
for pattern in "${BLOCKED_PATTERNS[@]}"; do
    if git diff --cached --name-only | grep -q "$pattern"; then
        echo "❌ 检测到不应提交的文件: $pattern"
...
if [ $? -ne 0 ]; then
    echo "❌ Checkstyle 检查未通过,请修复代码规范问题"
    exit 1
fi
...
<!-- 使用 Closes #xxx 关闭 Issue,或 Refs #xxx 仅引用 -->
Closes #
...
- [ ] 代码已通过本地构建 (`mvn clean verify`)
- [ ] 测试覆盖率 ≥ 80% (关键模块 ≥ 90%)
- [ ] 无 Checkstyle/SpotBugs 严重问题
- [ ] 无敏感信息泄露
- [ ] 已更新相关文档
```
