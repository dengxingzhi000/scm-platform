...
-- 插入调度锁
INSERT INTO `xxl_job_lock` (`lock_name`) VALUES ('schedule_lock') ON DUPLICATE KEY UPDATE lock_name='schedule_lock';

-- 插入 SCM 执行器组
INSERT INTO `xxl_job_group` (`id`, `app_name`, `title`, `address_type`, `address_list`, `update_time`)
VALUES
  (1, 'scm-order-executor', '订单服务执行器', 0, NULL, NOW()),
  (2, 'scm-inventory-executor', '库存服务执行器', 0, NULL, NOW())
...
INSERT INTO `xxl_job_info` (
  ...
  1, 1, '订单超时自动取消', NOW(), NOW(),
  'SCM Platform Team', '', 'CRON', '0 */1 * * * ?',
  'DO_NOTHING', 'FIRST', 'orderTimeoutCancelJobHandler',
  '30', 'SERIAL_EXECUTION', 300,
  3, 'BEAN', '', 'GLUE代码初始化',
  NOW(), '', 0, 0, 0
) ON DUPLICATE KEY UPDATE update_time=NOW();
...
INSERT INTO `xxl_job_info` (
  ...
  2, 2, '库存快照定时同步', NOW(), NOW(),
  'SCM Platform Team', '', 'CRON', '0 0 1 * * ?',
  'DO_NOTHING', 'FIRST', 'inventorySnapshotJobHandler',
  '', 'SERIAL_EXECUTION', 600,
  3, 'BEAN', '', 'GLUE代码初始化',
  NOW(), '', 0, 0, 0
) ON DUPLICATE KEY UPDATE update_time=NOW();